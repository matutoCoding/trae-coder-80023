...
export default function LockerPoolCard({ pool, compact = false }: { pool: LockerPool; compact?: boolean }) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl bg-gradient-to-br ${c.bg} ring-1 ${c.ring} p-4 transition-all`}>
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className={`w-9 h-9 rounded-lg bg-industrial-900/60 flex items-center justify-center ${c.text}`}>
            <Box size={18} />
          </div>
          <div>
            <p className={`font-semibold ${c.text}`}>{SIZE_LABEL[pool.size]}格口</p>
            {!compact && <p className="text-[11px] text-industrial-400">{SIZE_DESC[pool.size]}</p>}
          </div>
        </div>
        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${low ? "bg-rose-500/20 text-rose-400" : "bg-emerald-500/20 text-emerald-400"}`}>
          {low ? "紧张" : "充足"}
        </span>
      </div>

      <div className="flex items-baseline gap-2 mb-2">
        <span className="text-3xl font-bold font-display text-white">{pool.available}</span>
      ...
      <div className="w-full h-1.5 bg-industrial-900/50 rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-500 ${low ? "bg-rose-500" : "bg-emerald-500"}`}
          style={{ width: `${percent}%` }}
        />
      </div>
